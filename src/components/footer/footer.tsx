...
import { getGuitarModal, getShowModalCardAdd, getShowModalCardDel, getShowModalSuccess } from '../../store/modal-data/selectors';
import { useEffect } from 'react';
import ModalCardDel from '../modal-card-del/modal-card-del';

const mapStateToProps = (state: State) => ({
  guitarModal: getGuitarModal(state),
  showModalCardAdd: getShowModalCardAdd(state),
  showModalCardDel: getShowModalCardDel(state),
  showModalSuccess: getShowModalSuccess(state),
});
...
function Footer({ guitarModal, onSetParams, onLoadGuitars, showModalCardAdd, showModalSuccess, showModalCardDel }: PropsFromRedux): JSX.Element {

  useEffect(() => {
    const body = document.querySelector('body');
    if (body) {
      body.style.overflow = showModalCardAdd || showModalSuccess || showModalCardDel  ? 'hidden' : 'auto';
    }
  }, [showModalCardAdd, showModalSuccess, showModalCardDel]);
      ...
      <footer className="footer">
        <div className="footer__container container">
          ...
          <section className="footer__nav-section footer__nav-section--info">
            ...
            <p className="footer__nav-content footer__nav-content--font-secondary">Магазин гитар, музыкальных инструментов и гитарная мастерская
              <br/> в Санкт-Петербурге.<br/><br/>Все инструменты проверены, отстроены <br/> и доведены до идеала!
            </p>
          </section>
